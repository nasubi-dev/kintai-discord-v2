...
async function listGuilds() {
  ...
  if (!BOT_TOKEN) {
    console.error(
      "❌ エラー: DISCORD_TOKEN または DISCORD_BOT_TOKEN 環境変数が設定されていません"
    );
    console.log("💡 解決方法:");
    console.log("   export DISCORD_TOKEN=your_bot_token_here");
    console.log("   または");
    console.log(
      "   DISCORD_TOKEN=your_bot_token_here bun run scripts/list-guilds.js"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Discord API エラー: ${response.status} ${response.statusText}\n${errorText}`
      );
    }
    ...
    if (guilds.length === 0) {
      ...
      console.log(
        "💡 Botを招待するには、Discord Developer Portal で招待URLを生成してください"
      );
      return;
    }
    ...
    for (const guild of guilds) {
      ...
      console.log(
        `│ ${name.padEnd(39)} │ ${guild.id.padEnd(20)} │ ${memberCount.padEnd(
          10
        )} │ ${permissionText.padEnd(12)} │`
      );
    }
    ...
    console.log(
      `   • サーバー管理権限を持つサーバー: ${manageGuilds.length} 個`
    );
    console.log(
      `   • 一般権限のサーバー: ${
        guilds.length - adminGuilds.length - manageGuilds.length
      } 個`
    );
...
async function checkConfiguredGuilds(guilds) {
  ...
  // 現在は基本的な情報のみ表示
  console.log(
    "   （KVストレージアクセスが必要なため、詳細確認はスキップされました）"
  );
  console.log(
    "   詳細な設定状況は Discord で `/config` コマンドを使用して確認してください"
  );
}

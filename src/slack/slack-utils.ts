/**
 * Slackリクエスト署名検証
 * @param slackSignature X-Slack-Signature
 * @param slackTimestamp X-Slack-Request-Timestamp
 * @param rawBody リクエストボディ（string）
 * @param signingSecret Slack AppのSigning Secret
 * @returns 署名が正しければtrue
 */
export async function verifySlackRequest(
  ...
  const reqTime = parseInt(slackTimestamp, 10);
  if (Math.abs(now - reqTime) > 60 * 5) return false;
  ...
  const sigData = encoder.encode(sigBase);
  ...
  const signatureBytes = await crypto.subtle.sign("HMAC", cryptoKey, sigData);
  ...
  return timingSafeEqual(expected, slackSignature);
}

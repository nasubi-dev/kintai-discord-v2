...
Discord を使用して勤怠管理を行うボットです。Hono フレームワークを使用して Cloudflare Workers 上で動作し、各サーバー管理者が個別に Google OAut7. **KV 関連のエラー**
...
9. **設定リセットが必要な場合**で、データの保存には Google スプレッドシートを使用します。
...
- リアルタイム労働時間計算
- Discord API による実際のチャンネル名取得（権限不足時はフォールバック対応）
...
| プロジェクト名 | チャンネル名（権限不足時はID） | kintai または channel-713082   |
...
- **🆕 時刻指定エラー**:
  ...
  - 開始時刻が現在時刻より未来
  - 終了時刻が開始時刻より前
- **チャンネル名取得**: Discord API権限不足時は自動的にフォールバック名（channel-XXXXXX）を使用
...
6. **チャンネル名がchannel-XXXXXXと表示される**

   - Botにチャンネルの「表示」権限が付与されているか確認
   - 管理者権限または「チャンネルを見る」権限をBotに付与
...
- **Discord API権限の改善**: チャンネル名取得の安定性向上
